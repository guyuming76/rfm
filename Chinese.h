...
"    在上述非rfm内置命令后面加上\033[33m&\033[0m后缀,会打开新的终端模拟器窗口来执行此命令.注意这里&后缀的作用是rfm定制的,不是linux默认的把命令放在后台执行. 对于执行完不等待用户输入就终止,导致终端模拟器关闭的命令,如ls&,可以运行ls;read&,以便看到输出后按回车再关闭窗口.也可以在config.h文件里把默认的&换成别的字符串\n" \
